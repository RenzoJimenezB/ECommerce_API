...
export class UsersService {
  ...
  async createAdmin(adminData: CreateAdminDto): Promise<PublicUserDto> {
    const dbAdmin: User = await this.usersRepository.findByEmail(adminData.email);

    if (dbAdmin)
      throw new BadRequestException("Email already exists");

    const { passwordConfirmation, ...filteredData } = adminData;
    const newAdmin: User = await this.usersRepository.createAdmin(filteredData);
  ...
  async findOne(
    ...
    requestingUserRole: UserRole): Promise<PublicUserDto> {
    ...
    if (id !== requestingUserId && ![UserRole.SUPERADMIN, UserRole.ADMIN].includes(requestingUserRole))
