...
export class UsersRepository {
  ...
  async createAdmin(adminData: Omit<CreateAdminDto, "passwordConfirmation">): Promise<User> {
    return this.repository.save({
      ...adminData,
  ...
  async createUser(userData: Omit<CreateUserDto, "passwordConfirmation">): Promise<User> {
    return this.repository.save(userData);
  } // check auth module and remove
  ...
  async update(id: string, updateData: UpdateUserDto): Promise<User> {
    ...
    const updatedUser: User = await this.repository.findOneBy({ id });
